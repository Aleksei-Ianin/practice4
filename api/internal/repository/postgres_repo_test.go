...
func TestMockRepoGetAll(t *testing.T) {
	repo := &mockRepo{calcs: make(map[int]domain.Calculation)}
	repo.Create(&domain.Calculation{Operand1: 1, Operand2: 2, Operation: "add", Result: 3})
	repo.Create(&domain.Calculation{Operand1: 4, Operand2: 5, Operation: "multiply", Result: 20})
	all, err := repo.GetAll()
	if err != nil {
		t.Fatal(err)
	}
...
func TestMockRepoDelete(t *testing.T) {
	repo := &mockRepo{calcs: make(map[int]domain.Calculation)}
	repo.Create(&domain.Calculation{Operand1: 7, Operand2: 8, Operation: "subtract", Result: -1})
	repo.Delete(0)
	all, _ := repo.GetAll()
	if len(all) != 0 {
